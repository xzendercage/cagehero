...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "No name set yet nibba, check pinned in @cageherosupport"
...
async def _(event):
    ...
    packname = f"{user.first_name}'s @cageherosupport Vol.{pack}"
    packshortname = f"vol_{pack}_with_{userid}"
    await event.edit("`Ruko Sar Sticker Copy Karlu!`**（　ﾟДﾟ）**\n`Ho GYa Bass Copy`**ヽ༼ ಠ益ಠ ༽ﾉ**")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "Cage25_roxx.png"
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if not response.text.startswith("Alright!"):
                if "unacceptable" in response.text:
                    packname = f"{user.id}'s @cageherosupport Vol.{pack}"
                    response = await silently_send_message(bot_conv, packname)
                else:
                    await event.edit(f"**FAILED**! @Stickers replied: {response.text}")
    ...
    await event.edit(f"**Copied!** `Sticker Pack Yaha Hai` [this place](t.me/addstickers/{packshortname}), pack{pack}"
                     f" `by` {DEFAULTUSER}\n**ᕦ(ò_óˇ)ᕤ**")
